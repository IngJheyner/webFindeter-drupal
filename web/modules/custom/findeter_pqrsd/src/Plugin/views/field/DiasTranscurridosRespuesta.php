...
/**
 * @file
 * Definition of Drupal\d8views\Plugin\views\field\DiasTranscurridosRespuesta
 */
...
class DiasTranscurridosRespuesta extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    ...
    if(isset($row->getFields()['field_pqrsd_fecha_respuesta'][0])){
        ...
        $fecha_inicio = new \DateTime(date('Y-m-d',$row->get('created')->getValue()[0]['value']));
        $fecha_fin =  new \DateTime($row->get('field_pqrsd_fecha_respuesta')->getValue()[0]['value']);

        $dias = $fecha_inicio->diff($fecha_fin);

        return $this->t('@days dias.', ['@days' => $dias->days]);
    }
